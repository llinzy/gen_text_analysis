...
transposed2=transposed.reset_index()
transposed2.columns=transposed2.iloc[0,:]
transposed2.columns=['skill_category','Critical_Thinking', 'Instructing', 
                     'Management_of_Financial_Resources',  'Mathematics',
                      'Quality_Control_Analysis',  'Service_Orientation', 
                     'Speaking','Technology_Design']
transposed2=transposed2.drop('skill_category', axis=1)
transposed2=transposed2.iloc[1:,:]

cls_X=st.selectbox('X', transposed2.columns,index=1)
cls_Y=st.selectbox('Y', transposed2.columns,index=4)
cls_fig = px.scatter(transposed2, x=cls_X, y=cls_Y)
